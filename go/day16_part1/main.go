...
func (d Dancers) get_dancers() string {
	...
	for index := 0; index < d.size; index++ {
		name := d.list[(d.head+index)%d.size].name
...
func solve(dancers Dancers, dance_moves []DanceMove) string {
...
func solution(filename string, size int) string {
	...
	return solve(dancers, dance_moves)
}

func main() {
	fmt.Println(solution("./example.txt", 5)) // "baedc"
	fmt.Println(solution("./input.txt", 16))  // "dcmlhejnifpokgba"
}
