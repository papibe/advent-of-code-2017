...
func solve(diagram Diagram) string {
	path := []string{}
	...
	for _, coord := range traverse(0, start_col, diagram) {
		current := diagram[coord.row][coord.col]
		if is_alpha(string(current)) {
...
func main() {
	fmt.Println(solution("./example.txt")) // "ABCDEF"
	fmt.Println(solution("./input.txt"))   // "EOCZQMURF"
}
