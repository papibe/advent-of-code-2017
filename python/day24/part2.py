...
def solve(bridge: List[Component], top: int, cmap: Dict[int, Set[Component]]) -> int:
    ...
    bridges: Dict[int, List[List[Component]]] = {}

    def _solve(
        bridge: List[Component], top: int, cmap: Dict[int, Set[Component]]
    ) -> None:
        if not cmap or top not in cmap or not cmap[top]:
            length: int = len(bridge)
            if length not in bridges:
                bridges[length] = []
            bridges[length].append(bridge.copy())
    ...
    _solve(bridge, top, cmap)
    max_length: int = max(bridges.keys())

    return max([get_bridge_strength(b) for b in bridges[max_length]])
